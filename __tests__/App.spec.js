...
describe("Transactions List", () => {
  ...
  it("should render the transactions list", () => {
    ...
    expect(tree.UNSAFE_getAllByType(TransactionItem).length).toBe(
      transactions.length
    );
  });
  ...
  it("should render the Transaction List Item correctly", () => {
    transactions.map((transaction) => {
      ...
      expect(tree.getByTestId(TEST_IDS.AMOUNT)).toHaveTextContent(
        transaction.amount
      );
      expect(tree.getByTestId(TEST_IDS.CITY)).toHaveTextContent(
        transaction.location.city
      );
      expect(tree.getByTestId(TEST_IDS.ADDRESS)).toHaveTextContent(
        transaction.location.address
      );
      expect(tree.getByTestId(TEST_IDS.DATE)).toHaveTextContent(
        transaction.date
      );
    });
  });
  ...
  it("should filter the list based on filter button click", async () => {
    ...
    expect(app.UNSAFE_getAllByType(TransactionItem).length).toBe(
      transactions.length
    );

    await fireEvent.press(app.getByTestId(TEST_IDS.CREDIT_BUTTON));
    expect(app.UNSAFE_getAllByType(TransactionItem).length).toBe(
      transactions.filter((t) => t.txnType === "credit").length
    );
    ...
    expect(app.UNSAFE_getAllByType(TransactionItem).length).toBe(
      transactions.filter((t) => t.txnType === "debit").length
    );
